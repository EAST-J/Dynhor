...
if __name__ == '__main__':
    ...
    # Init Dinov2
    dino_model = Dinov2("/remote-home/jiangshijian/facebookresearch_dinov2_main", source='local')
    # Load 3D prior
    ...
    if config['random_render']:
        prior_batched_renderings, prior_depths, Rs, Ts, Ks = batch_random_render(device='cuda', mesh=obj_mesh.to('cuda'), 
                                                                        view_numbers=2000, # You can adjust the view_numbers to save the memory usage.
                                                                        H=RENDER_H, W=RENDER_W, distance_scale=2.5)
    else:
    ...
    # For debug
    view_selection_dirs = os.path.join(sample_folder, "init_views")
    os.makedirs(view_selection_dirs, exist_ok=True)
    os.makedirs(os.path.join(sample_folder, "rendering_views"), exist_ok=True)
    for idx, prior_rendering in enumerate(prior_batched_renderings):
        cv2.imwrite(os.path.join(sample_folder, "rendering_views/view_{:04d}.jpg".format(idx)), prior_rendering.cpu().numpy()*255)
    ...
    object_parameters = find_optimal_poses(
        ...
        view_selection_dirs=view_selection_dirs, # For Debug
    )
