...
class ProgressiveBandHashGrid(nn.Module): # ？这个过程是否以及有点类似于BARF？
    def __init__(self, in_channels, config):
        super().__init__()
...
class VanillaMLP(nn.Module):
    ...
    def reinitialize_weights(self):
        """
        Reinitialize all layers with the same initialization used in make_linear.
        """
        for i, layer in enumerate(self.layers):
            if isinstance(layer, nn.Linear):
                dim_in, dim_out = layer.in_features, layer.out_features
                is_first = (i == 0)
