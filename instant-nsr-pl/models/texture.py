...
from models.network_utils import get_encoding, get_mlp, inverse_sigmoid
...
# TODO: 关于网络的设计？
@models.register('volume-relight')
class VolumeRelight(nn.Module):
    def __init__(self, config):
        super(VolumeRelight, self).__init__()
        self.config = config
        self.n_dir_dims = self.config.get('n_dir_dims', 3)
        self.n_output_dims = 3
        encoding = get_encoding(self.n_dir_dims, self.config.dir_encoding_config)
        self.n_input_dims = self.config.input_feature_dim + encoding.n_output_dims # pts, dirs, normals
        self.in_layer = get_mlp(self.n_input_dims, self.config.inlayer_mlp_network_config.n_neurons, self.config.inlayer_mlp_network_config)    
        network = get_mlp(self.config.inlayer_mlp_network_config.n_neurons+3, self.n_output_dims, self.config.mlp_network_config)    
        self.encoding = encoding
        self.network = network
    
    def forward(self, rgb, dirs, *args):
        dirs = (dirs + 1.) / 2. # (-1, 1) => (0, 1)
        dirs_embd = self.encoding(dirs.view(-1, self.n_dir_dims))
        network_inp = torch.cat([dirs_embd] + [arg.view(-1, arg.shape[-1]) for arg in args], dim=-1)
        drgb = self.in_layer(network_inp).float()
        drgb = torch.cat([drgb, rgb], dim=-1)
        drgb = self.network(drgb)
        if 'color_activation' in self.config:
            color = get_activation(self.config.color_activation)(inverse_sigmoid(rgb) + drgb)
        return color, drgb
