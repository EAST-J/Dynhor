...
@models.register('color-neus')
class ColorNeuSModel(BaseModel):
    def setup(self):
        self.geometry = models.make(self.config.geometry.name, self.config.geometry)
        self.global_texture = models.make(self.config.global_texture.name, self.config.global_texture)
    ...
    def update_step(self, epoch, global_step):
        ...
        update_module_step(self.global_texture, epoch, global_step)
        if self.config.learned_background:
    ...
    def forward_(self, rays):
        ...
        if positions.shape[0] == 0 and (not self.training): # !issues: https://github.com/nerfstudio-project/nerfacc/issues/207
            comp_rgb = torch.zeros((n_rays, 3), device=rays.device)
            comp_normal = torch.zeros((n_rays, 3), device=rays.device)
            opacity = torch.zeros((n_rays, 1), device=rays.device)
            depth = torch.zeros((n_rays, 1), device=rays.device)
            delta_rgb = torch.zeros((n_rays, 3), device=rays.device)
        else:
            if self.config.geometry.grad_type == 'finite_difference':
                sdf, sdf_grad, feature, sdf_laplace = self.geometry(positions, with_grad=True, with_feature=True, with_laplace=True, cat_pts=True)
                # sdf, sdf_grad, feature, sdf_laplace = self.geometry(positions, with_grad=True, with_feature=True, with_laplace=True)
            else:
                sdf, sdf_grad, feature = self.geometry(positions, with_grad=True, with_feature=True, cat_pts=True)
                # sdf, sdf_grad, feature = self.geometry(positions, with_grad=True, with_feature=True)
            normal = F.normalize(sdf_grad, p=2, dim=-1)
            alpha = self.get_alpha(sdf, normal, t_dirs, dists)[...,None]
            global_rgb = self.global_texture(feature, normal)
            rgb, delta_rgb = self.texture(global_rgb, t_dirs, normal, feature[:, -3:])

            weights = render_weight_from_alpha(alpha, ray_indices=ray_indices, n_rays=n_rays)
            opacity = accumulate_along_rays(weights, ray_indices, values=None, n_rays=n_rays)
            # depth = accumulate_along_rays(weights, ray_indices, values=midpoints, n_rays=n_rays) / (opacity + 1e-6)
            depth = accumulate_along_rays(weights, ray_indices, values=midpoints, n_rays=n_rays)
            comp_rgb = accumulate_along_rays(weights, ray_indices, values=rgb, n_rays=n_rays)

            comp_normal = accumulate_along_rays(weights, ray_indices, values=normal, n_rays=n_rays)
            comp_normal = F.normalize(comp_normal, p=2, dim=-1)
            # some changes from Instant-angelo 
            rays_fg = opacity > 0.1
        ...
        out = {
            ...
            'opacity': opacity,
            'depth': depth,
            'surface_pts': rays_o + rays_d * depth,
            'rays_valid': opacity > 0,
            'delta_rgb': delta_rgb,
            'num_samples': torch.as_tensor([len(t_starts)], dtype=torch.int32, device=rays.device)
        }
    ...
    def regularizations(self, out):
        ...
        losses.update(self.global_texture.regularizations(out))
        return losses
    ...
    def export(self, export_config):
        ...
        if export_config.export_vertex_color:
            ...
            _, sdf_grad, feature = chunk_batch(self.geometry, export_config.chunk_size, False, mesh['v_pos'].to(self.rank), with_grad=True, with_feature=True, cat_pts=True)
            normal = F.normalize(sdf_grad, p=2, dim=-1)
            rgb = self.global_texture(feature, normal)
            # rgb = self.global_texture(feature, normal, mesh['v_pos'].to(self.rank))
            mesh['v_rgb'] = rgb.cpu()
        return mesh
