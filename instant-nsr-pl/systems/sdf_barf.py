...
@systems.register('sdf-barf-system')
class SDFSystem(BaseSystem):
    """
    ...
    def training_step(self, batch, batch_idx):
        ...
        # depth loss
        # ! 需要考虑到每frames的depth的scale和shift要分开优化
        ...
        # if hasattr(self.config.system.loss, 'lambda_sdf_l1') and self.C(self.config.system.loss.lambda_sdf_l1) > 0:
        #     temp_v = batch['temp_v']
        #     pts2sdf = self.model.geometry(temp_v, with_grad=False, with_feature=False)
        #     loss_sdf = F.l1_loss(pts2sdf, torch.zeros_like(pts2sdf))
