...
class SaverMixin():
    ...
    def save_mesh(self, filename, v_pos, t_pos_idx, v_tex=None, t_tex_idx=None, v_rgb=None, post_process=False):
        ...
        if post_process:
            connected_comp = mesh.split(only_watertight=True)
            dis_min = 1e3
            select_comp = None
            # mean_area = np.mean([comp.area for comp in connected_comp])
            for comp in connected_comp:
                dis_mean = np.linalg.norm(comp.vertices, axis=1).min()
                if dis_mean < dis_min:
                    dis_min = dis_mean
                    select_comp = comp
            mesh = select_comp
