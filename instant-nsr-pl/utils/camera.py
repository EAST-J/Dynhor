...
def convert_opengl_to_opencv(w2c_opengl):
    """
    将 OpenGL 坐标系中的 world2cam 矩阵（大小为 b×3×4）转换为 OpenCV 坐标系中的 world2cam 矩阵。
    
    参数：
    w2c_opengl -- OpenGL 坐标系中的 world2cam 矩阵，大小为 b×3×4
    
    返回：
    w2c_opencv -- OpenCV 坐标系中的 world2cam 矩阵，大小为 b×3×4
    """
    b = w2c_opengl.shape[0]
    
    # 构造转换矩阵（反转 y 轴和 z 轴）
    M_opengl_to_opencv = torch.eye(3).unsqueeze(0).repeat(b, 1, 1).to(w2c_opengl.device)  # b×3×3的单位矩阵
    
    # 反转 y 轴
    M_opengl_to_opencv[:, 1, 1] = -1
    
    # 反转 z 轴
    M_opengl_to_opencv[:, 2, 2] = -1
    
    # 提取原来的平移部分（最后一列）
    translation = w2c_opengl[:, :, 3:]  # b×3×1
    
    # 提取原来的旋转部分（前 3×3 部分）
    rotation = w2c_opengl[:, :, :3]  # b×3×3
    
    # 对旋转部分应用坐标系转换
    rotated_w2c = torch.matmul(M_opengl_to_opencv, rotation)
    
    # 更新平移部分：对每个平移向量应用转换矩阵
    transformed_translation = torch.matmul(M_opengl_to_opencv, translation)
    
    # 组合旋转和平移部分
    w2c_opencv = torch.cat([rotated_w2c, transformed_translation], dim=2)  # b×3×4
